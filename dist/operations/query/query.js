...
exports.ElendMarketQueryOperation = void 0;
const utils_1 = require("../../utils");
class ElendMarketQueryOperation {
    constructor(networkConfig, client) {
        this.suiClient = client;
        this.networkConfig = networkConfig;
    }
    async fetchObligationOwnerCapObject(owner, marketType) {
        ...
        if (response.data.length > 0 && response.data[0]?.data?.content) {
            const data = (response.data[0]?.data?.content).fields;
            return {
                id: data.id.id,
    ...
    async fetchReserve(reserveId) {
        ...
        if (response.data?.content) {
            const reserveData = (response.data?.content)['fields'];
            ...
            return {
                ...
                liquidity: {
                    ...
                    borrowedAmount: new utils_1.Decimal(BigInt(reserveLiquidityData.borrowed_amount.fields.value)),
                    marketPrice: new utils_1.Decimal(BigInt(reserveLiquidityData.market_price.fields.value)),
                    ...
                    cumulativeBorrowRate: new utils_1.Decimal(BigInt(reserveLiquidityData.cumulative_borrow_rate.fields.value)),
                    accumulatedProtocolFees: new utils_1.Decimal(BigInt(reserveLiquidityData.accumulated_protocol_fees.fields.value)),
                },
                collateral: {
                ...
                config: {
                    ...
                    depositWithdrawalCap: {
                        configCapacity: (0, utils_1.i64ToBigInt)(reserveConfigData.deposit_withdrawal_cap.fields.config_capacity.fields.magnitude, reserveConfigData.deposit_withdrawal_cap.fields.config_capacity.fields.negative),
                        currentTotal: (0, utils_1.i64ToBigInt)(reserveConfigData.deposit_withdrawal_cap.fields.current_total.fields.magnitude, reserveConfigData.deposit_withdrawal_cap.fields.current_total.fields.negative),
                    ...
                    debtWithdrawalCap: {
                        configCapacity: (0, utils_1.i64ToBigInt)(reserveConfigData.debt_withdrawal_cap.fields.config_capacity.fields.magnitude, reserveConfigData.debt_withdrawal_cap.fields.config_capacity.fields.negative),
                        currentTotal: (0, utils_1.i64ToBigInt)(reserveConfigData.debt_withdrawal_cap.fields.current_total.fields.magnitude, reserveConfigData.debt_withdrawal_cap.fields.current_total.fields.negative),
                    ...
                    minNetValueInObligation: new utils_1.Decimal(reserveConfigData.min_net_value_in_obligation.fields.value),
                },
                lastUpdate: {
    ...
    async fetchObligation(obligationId) {
        ...
        if (response.data?.content) {
            ...
            return {
                ...
                totalDepositedValue: new utils_1.Decimal(data.total_deposited_value.fields.value),
                borrowFactorAdjustedDebtValue: new utils_1.Decimal(data.borrow_factor_adjusted_debt_value.fields.value),
                highestBorrowFactorBps: Number(data.highest_borrow_factor_bps),
                allowedBorrowValue: new utils_1.Decimal(data.allowed_borrow_value.fields.value),
                unhealthyBorrowValue: new utils_1.Decimal(data.unhealthy_borrow_value.fields.value),
    ...
    async fetchRewardConfigs(reserveId, marketType, option, rewardTokenType) {
        const packageInfo = this.networkConfig.packages[this.networkConfig.latestVersion];
        ...
        for (const rewardConfigDf of rewardConfigDfs.data) {
            if (rewardConfigDf.objectType == `vector<${packageInfo.package}::reward_config::RewardConfig<${marketType}>>` &&
                rewardConfigDf.name.value.option == option) {
                ...
                if (rewardConfigRes.data?.content) {
                    const data = (rewardConfigRes.data?.content).fields.value;
    ...
    async fetchUserReward(reserveId, rewardTokenType, option, obligationId, owner) {
        const packageInfo = this.networkConfig.packages[this.networkConfig.latestVersion];
        const userRewardKey = {
            owner,
            reserve: reserveId,
...
exports.ElendMarketQueryOperation = ElendMarketQueryOperation;
